...
@Component({
  selector: 'app-esiti-istruttoria-zootecnia',
  templateUrl: './esiti-istruttoria-zootecnia.component.html',
  styleUrls: ['../esiti-istruttoria/esiti-istruttoria.component.css']
})

export class EsitiIstruttoriaZootecniaComponent implements OnInit, OnDestroy {
  ...
  private interventi: string[] = ['int310', 'int311', 'int313', 'int322', 'int315', 'int316', 'int318', 'int320', 'int321'];
  private interventiRichiesti: any[];
  private vedereAncheSeUgualiAZero: string[] = ['DFFRAPPDIS_INPUT', 'DFFRAPPACZ_INPUT', 'DFFRAPPACS_INPUT', 'DFIMPLIQACZLORDO_OUTPUT'];
  ...
  private setObject() {
    this.datiView = [
      ...
      { tipoEsito: this.interventi[8], datiInput: [], datiOutput: [] },
    ];
  ...
  private getDatiIstruttoria() {
    ...
    this.service.getEsitiCalcoloIstruttoriaAcz(this.idIstruttoria.toString()).pipe(
      switchMap((dati) => {
        if (dati) {
        ...
        return this.service.getDisciplinaIstruttoriaAcz(this.idIstruttoria.toString());
      }),
      catchError((error) => {
        if (error.error.message === 'NO_CALCOLO_ACZ') {
          console.log('Non ci sono passi lavorazione ACZ validi per l\'istruttoria con id: ' + this.idIstruttoria.toString());
        } else {
