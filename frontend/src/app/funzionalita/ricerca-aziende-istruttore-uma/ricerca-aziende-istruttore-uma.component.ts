...
import { PopupNuovoPrelievoComponentIstruttoreUMA } from './popup-nuovo-prelievo-istruttore-uma/popup-nuovo-prelievo-istruttore-uma.component';
...
@Component({
  selector: 'app-ricerca-aziende-istruttore-uma',
  templateUrl: './ricerca-aziende-istruttore-uma.component.html',
  styleUrls: ['./ricerca-aziende-istruttore-uma.component.scss']
})
export class RicercaAziendeComponentIstruttoreUMA implements OnInit, OnDestroy {

  @ViewChild("popupNuovoPrelievo")
  popupNuovoPrelievo: PopupNuovoPrelievoComponentIstruttoreUMA;

  a4gMessages = A4gMessages;
  umaMessages = UMA_MESSAGES;

  ricercaForm: FormGroup;
  querySearch: { cuaa: string, campagna: string };          /** modello della form di ricerca */
  querySearchRicercaDisponibilita: { cuaa: string, idDomanda: string };
  ...
  onCloseDialog($event: { type: 'SAVE' | 'UPDATE' | 'KO', model: PrelievoDto }) {
    if ($event.type === 'SAVE') {
      ...
      this.getRichiesteAndGetPrelievi({ cuaa: this.ricercaForm.get('cuaa').value, campagna: this.ricercaForm.get('campagna').value });
  ...
  ricercaDomanda(query?: { cuaa: string, campagna: string }) {
    const params: DomandaUmaFilter = {
      cuaa: query ? query.cuaa : this.ricercaForm.get('cuaa').value,
      campagna: this.ricercaForm.get('campagna').value,
  ...
  private getPrelievi(cuaa: string) {
    return this.httpClientDomandaUmaService.getPrelieviByCuaaAndCampagna(cuaa, this.ricercaForm.get('campagna').value);
  }

  private getRichiesteAndGetPrelievi(query?: { cuaa: string, campagna: string }) {
  ...
  getDichiarazioneConsumiAutorizzata(query?: { cuaa: string, campagna: string }) {
  ...
  private setRichieste(richieste: PaginatorA4G<Array<RichiestaCarburanteDto>>) {
    if (richieste && richieste.count > 0) {
      this.querySearch = { cuaa: this.ricercaForm.get('cuaa').value, campagna: this.ricercaForm.get('campagna').value };
      this.querySearchRicercaDisponibilita = { cuaa: this.ricercaForm.get('cuaa').value, idDomanda: richieste.risultati[0].id.toString() };
  ...
  private setPrelievi(prelievi: Array<PrelievoDto>) {
    if (prelievi && prelievi.length) {
      ...
      //rendo tutti i prelievi modificabili
      this.setReadOnlyFalse();
    } else {
      this.datasourcePrelievi = [];
    }
  }
  ...
  setReadOnlyFalse() {
    this.datasourcePrelievi.forEach(prelievo => prelievo.readonly = false);
  }
  ...
  private initData() {
    ...
    this.distributoriVM = [];
    this.valoriCampagna = [];
    // carica i distributori della campagna attuale
    this.getDistributoriSubscription = this.httpClientUtenteService.getDistributori(this.dateUtilService.getCurrentYear().toString()).subscribe((distributori: Array<UtenteDistributoreDto>) => {
