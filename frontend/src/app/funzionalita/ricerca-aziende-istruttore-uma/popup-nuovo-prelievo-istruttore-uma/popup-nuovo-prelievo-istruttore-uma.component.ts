...
import { PresentaPrelievoDto } from 'src/app/uma/core-uma/models/dto/PresentaPrelievoDto';
import { FormService } from 'src/app/a4g-common/services/form.service';
@Component({
  selector: 'app-popup-nuovo-prelievo-istruttore-uma',
  templateUrl: './popup-nuovo-prelievo-istruttore-uma.component.html',
  styleUrls: ['./popup-nuovo-prelievo-istruttore-uma.component.scss']
})
export class PopupNuovoPrelievoComponentIstruttoreUMA implements OnInit, OnDestroy {
  ...
  open(prelievoLoaded: { idRichiesta: string, prelievo: PrelievoViewModel, prelevabile: CarburanteDto , isDichiarazioneConsumiAutorizzata: boolean}) {
    ...
    //i prelievi non sono modificabili solo se la dichiarazione consumi è stata autorizzata
    this.READONLY_MODE = prelievoLoaded && prelievoLoaded.prelievo && prelievoLoaded.isDichiarazioneConsumiAutorizzata ? true : false;
