...
public class RicercaFascicoloService {
	...
	public RisultatiPaginati<FascicoloDto> ricerca(FascicoloFilter criteri,	Paginazione paginazione, Ordinamento ordinamento)  {
		criteri.setEntiUtenteConnesso(caaService.getSportelliAbilitatiCaa());
		Page<FascicoloModel> page = fascicoloDao.findAll(
				FascicoloSpecificationBuilder.getFilter(criteri),
		...
		if (risultati != null) {
			risultati.forEach(fascicoloDto -> {
				Optional<FascicoloModel> fascicoloModelOptional = fascicoloDao.findByCuaaAndIdValidazione(fascicoloDto.getCuaa(), 0);
