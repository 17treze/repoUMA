...
public class FascicoloAgsRowMapper implements RowMapper<FascicoloAgsRow> {
	...
	public FascicoloAgsRow mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		if (statoLegacy.startsWith("VALID")) {
		...
		return fascicoloAgsRow.setIdAgs(rs.getLong("ID")).setCuaa(rs.getString("CUAA"))
				.setDenominazione(rs.getString("DENOMINAZIONE"))
				.setDataMorte(
						rs.getTimestamp("DATA_MORTE") != null ? rs.getTimestamp("DATA_MORTE").toLocalDateTime() : null)
				.setOrganismoPagatore(rs.getString("ORGANISMO_PAGATORE")).setCaa(rs.getString("CAA"))
				...
				.setDataValidazione(rs.getTimestamp("DATA_VALIDAZIONE") != null
						? rs.getTimestamp("DATA_VALIDAZIONE").toLocalDateTime()
						: null)
				.setDataInizioDetenzione(rs.getTimestamp("DATA_INIZIO_DETENZIONE") != null
						? rs.getTimestamp("DATA_INIZIO_DETENZIONE").toLocalDateTime()
						: null)
				.setDataFineDetenzione(rs.getTimestamp("DATA_FINE_DETENZIONE") != null
						? rs.getTimestamp("DATA_FINE_DETENZIONE").toLocalDateTime()
						: null)
