...
public class AbilitazioniComponent {
	...
	private boolean isGestioneFascicoloAutonomo(final String cuaa) {
		return checkApriFascicoloAutogestito() && isAventeDirittoFirma(cuaa)
	...
	private boolean auxCheckFascicoloOperation(final String cuaa, final Integer idValidazione,
			final EnteOperationType enteOperationType) {
		if (cuaa == null || idValidazione == null) {
			throw new NullPointerException();
		}
		Ruoli ruolo = enteOperationType.equals(EnteOperationType.RICERCA) ? Ruoli.RICERCA_FASCICOLO_ENTE
				: Ruoli.APERTURA_FASCICOLO_ENTE;
	...
	private boolean checkFascicoloAltroOp(final String cuaa, final Integer idValidazione) {
		try {
			...
			if (fascicoloModel.getOrganismoPagatore().equals(OrganismoPagatoreEnum.ALTRO_OP)
					&& fascicoloModel.getStato().equals(StatoFascicoloEnum.TRASFERITO)
	...
	public boolean checkLetturaFascicolo() {
		return utenteComponent.haUnRuolo(Ruoli.RICERCA_FASCICOLO_FILTRO_ENTE, Ruoli.RICERCA_FASCICOLO_FILTRO_UTENTE,
				Ruoli.RICERCA_FASCICOLO_NON_FILTRATA);
	}
	
	public boolean checkLetturaCuaa(String cuaa) throws Exception {
		if (utenteComponent.haUnRuolo(Ruoli.RICERCA_FASCICOLO_NON_FILTRATA))
			return true;
	...
	public boolean checkLetturaFascicolo(final String cuaa, final Integer idValidazione) {
		...
		if (utenteComponent.haUnRuolo(Ruoli.RICERCA_FASCICOLO_TUTTI)
				|| utenteComponent.haUnRuolo(Ruoli.VISUALIZZAZIONE_FASCICOLO_SOLA_LETTURA_TUTTI)) {
			return true;
		}
	...
	public boolean checkLetturaFascicoloPerRevocaImmediata(final String idProtocollo) {
		...
		try {
			...
			Optional<RevocaImmediataModel> revocaImmediataModelOpt = revocaImmediataDao
					.findByIdProtocollo(idProtocollo);
			...
			return checkLetturaFascicolo(detenzioneModel.getFascicolo().getCuaa(),
					detenzioneModel.getFascicolo().getIdValidazione());
		}
		catch (Exception e) {
			return false;
		}
	...
	/**
	 * LORENZO MARTINELLI Questo codice non ha alcun senso. In particolare non va bene chiamare getEntiUtenteConnesso
	 * che ritorna la lista di id degli sportelli che saranno sempre diversi da tutti i CUAA MA non va neppure usato
	 * getAziendeUsingGET in anagraficaUtenteClient perchè è una replica di dati in a4gutente che DEVE ESSERE ELIMINATA.
	 * Owner dell'informazione se una persona fisica è il titolare dell'azienda è il fascicolo ovvero il microservizio
	 * anagrafica
	 */
	public boolean isAccessoPersonaFisica(final String cuaa) {
		RappresentaIlModelloPerRappresentareUnUtenteDelSistema utenteConnesso = anagraficaUtenteClient
				.getUtenteConnesso();
	...
	public boolean checkPermessiAggiornamentoModoPagamento(final String cuaa) {
		if (utenteComponent.haUnRuolo(Ruoli.AGGIORNAMENTO_MODALITA_PAGAMENTO_FASCICOLO_ENTE)
				&& caaService.isAccessoCaa(cuaa, 0)) {
			return true;
		}
		return isGestioneFascicoloAutonomo(cuaa);
	}
	...
	public boolean checkPermessiVisualizzazioneModoPagamento(final String cuaa) {
		if (utenteComponent.haUnRuolo(Ruoli.VISUALIZZAZIONE_MODALITA_PAGAMENTO_FASCICOLO_ENTE)
				&& (caaService.isAccessoCaa(cuaa, 0) || hasProfiloResponsabileFascicoloPat())) {
			return true;
		}
	...
	public boolean checkEditDichiarazioniAssociative(final String cuaa) throws Exception {
		return checkLetturaDichiarazioniAssociative(cuaa)
				&& utenteComponent.haUnRuolo(Ruoli.DICHIARAZIONI_ASSOCIATIVE_EDITA_UTENTE);
	}
	...
	public boolean checkApriFascicoloAutogestito() {
		RappresentaIlModelloPerRappresentareUnUtenteDelSistema utenteConnesso = anagraficaUtenteClient
				.getUtenteConnesso();
		return fascicoloService.containsProfiloAzienda(utenteConnesso.getProfili())
				|| fascicoloService.containsProfiloAutomazioneApprovazione(utenteConnesso.getProfili());
	}
	...
	private boolean isFascicoloAbilitatoEnte(String cuaa) throws Exception {
		Optional<FascicoloAgsDto> fascicoloAgsOpt = Optional
				.ofNullable(fascicoloAgsService.getFascicolo(cuaa, LocalDateTime.now()));
		if (fascicoloAgsOpt.isPresent()) {
			List<Long> sportelliFascicolo = fascicoloAgsOpt.get().getDetenzioni().stream()
					.map(DetenzioneAgsDto::getIdentificativoSportello).collect(Collectors.toList());
			
			List<Long> sportelliUtente = abilitazioniUtente.getEntiUtente().stream().map(Long::valueOf)
			...
			if (!isAbilitato) {
				logger.warn("[ABILITAZIONE FALLITA ENTE] - Tentato Accesso al fascicolo {} da parte di {}", cuaa,
						abilitazioniUtente.getEntiUtente());
			}
			return isAbilitato;
		}
