...
@Component
public class CalcoloSostegnoM8SoiaConsumer extends CalcoloSostegnoAccoppiatoSuperficieConsumer {
	...
	/* M8
"IF (CTRLREGM8 NO OR CTRLCOLTM8 NO) THEN 0
ELSE IF (AZCMP =NO AND CTRLCOORD = SI AND CTRLREGM8 SI AND CTRLCOLTM8 SI) THEN MAX((PSSUPELEGIS-PFSUPANCOORD),0)
ELSE IF (AZCMP=SI AND AND CTRLCOORD = NO AND CTRLREGM8 SI AND CTRLCOLTM8 SI) THEN PFSUPCTRLLOCO
ELSE IF (AZCMP=SI AND CTRLCOORD = SI AND CTRLREGM8 SI AND CTRLCOLTM8 SI) THEN MAX((PFSUPCTRLLOCO-PFSUPANCOORD),0)
ELSE  PSSUPELEGIS"
"
	 */
	@Override
