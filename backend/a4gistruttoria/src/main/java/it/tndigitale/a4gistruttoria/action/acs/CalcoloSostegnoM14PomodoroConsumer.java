...
@Component
public class CalcoloSostegnoM14PomodoroConsumer
	...
	/*
"IF (CTRLREGM14 NO OR CTRLCOLTM14 NO) THEN 0
ELSE IF (AZCMP=NO CTRLCOORD = SI AND CTRLREGM14 SI AND CTRLCOLTM14 SI) THEN MAX((PSSUPELEGIS-PFSUPANCOORD),0)
ELSE IF (AZCMP=SI AND CTRLCOORD=NO AND CTRLREGM14 SI AND CTRLCOLTM14 SI)  THEN PFSUPCTRLLOCO
ELSE IF (AZCMP=SI AND CTRLCOORD=SI AND CTRLREGM14 SI AND CTRLCOLTM14 SI) THEN MAX((PFSUPCTRLLOCO-PFSUPANCOORD),0)
ELSE PSSUPELEGIS"
	 */
	@Override
	protected Float calcolaSuperficieDeterminata(RichiestaSuperficiePerCalcoloDto richiesta,
		...
		Float superficieEleggibileGis = super.getParticellaValue(richiesta, info,
				TipoVariabile.valueOf("ACSPFSUPELEGIS_".concat(suffissoMisura)), Float.class);
		Float superficieAnCoord = super.getParticellaValue(richiesta, info,
