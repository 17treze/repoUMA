...
public abstract class ProtocollazioneStrategy {
	...
	protected void salvaSuperficiMassime(RichiestaCarburanteModel richiesta) {
		...
		if (!CollectionUtils.isEmpty(raggruppamentiSuperficie)) {
			...
			superficieMassimaDao.save(new SuperficieMassimaModel() 
					.setRichiestaCarburante(richiesta)
					.setGruppoLavorazione(gruppiLavorazioneDao.findById(raggruppamento.getId()).orElseThrow(() -> new EntityNotFoundException(String.format("Raggruppamento con indice : %s non trovato", raggruppamento.getIndice()))))
	...
	protected String getEntePresentatore(FascicoloAgsDto fascicolo) {
		...
		var det = fascicolo.getDetenzioni().stream()
				...
				.orElseGet(() -> fascicolo.getDetenzioni().stream()
						.filter(detenzione -> fascicolo.getDetenzioni().size() == 1 && detenzione.getTipoDetenzione().equals(TipoDetenzioneEnum.DELEGA))
						.findFirst()
	...
	/**
	 * Reperisce i dati di un richiedente in base al tipo di domanda indicato. 
	 * La richiesta di carburante reperisce esclusivamente i Titolari e i Rappresentanti legali.
	 * La rettifica e la dichiarazione consumi reperiscono i Titolari/RL o in alternativa gli eventuali Eredi.
	 * @param cuaa
	 * @param cfRichiedente
	 * @param tipo
	 * @return
	...
	protected CaricaAgsDto reperisciDatiRichiedente(String cuaa, String cfRichiedente, TipoDocumentoUma tipo) {
		...
		if (TipoDocumentoUma.RICHIESTA.equals(tipo)) {
			...
			soggetti = fascicolo.getDataMorteTitolare() == null ? anagraficaClient.getTitolariRappresentantiLegali(cuaa) : anagraficaClient.getEredi(cuaa);
		}
		Assert.isTrue(!CollectionUtils.isEmpty(soggetti), String.format("Nessun soggetto trovato nel fascicolo per il cuaa %s", cuaa));

		return soggetti.stream()
				.filter(persona -> persona.getCodiceFiscale().equals(cfRichiedente))
				.findFirst().orElseThrow(() -> new IllegalArgumentException("La persona ".concat(cfRichiedente).concat(" non è presente nel fascicolo aziendale ").concat(cuaa)));
	}
	...
	protected void verificaFirmaDocumentoAndSave(ByteArrayResource document, String codiceFiscaleFirmatario, boolean haFirma) {
		try {
			verificaFirmaClient.verificaFirma(document, codiceFiscaleFirmatario);
		} catch (Exception e) {
			if (haFirma) {
