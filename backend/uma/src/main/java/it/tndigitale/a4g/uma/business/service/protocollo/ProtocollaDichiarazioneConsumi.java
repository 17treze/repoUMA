...
public class ProtocollaDichiarazioneConsumi extends ProtocollazioneStrategy {
	...
	public void avviaProtocollo(Long id, ByteArrayResource documento, boolean haFirma) {
		...
		ProtocollaDocumentoUmaDto protocollaDocumentoUmaDto = new ProtocollaDocumentoUmaDto().setDocumento(documento)
				.setId(id).setCuaa(richiesta.getCuaa()).setAnno(richiesta.getCampagna().intValue()).setNome("Nome1") // richiedente.getDescNome())
	...
	public DocumentDto buildDocumentDto(ProtocollaDocumentoUmaDto dati) {
		...
		ByteArrayResource documentoByteAsResource = getAllegatoAsByteArrayResource(dati.getDocumento().getByteArray(),
				String.valueOf(dati.getId()).concat(SUFFISSO_NOME_FILE_DICHIARAZIONE_CONSUMI).concat(".pdf"));
		// Metadati
		...
		String oggetto = PersonaSelector.isPersonaFisica(dati.getCuaa())
				? String.format(PREFISSO_OGGETTO_DICHIARAZIONE_CONSUMI + "%s - %s - %s %s", dati.getAnno(),
						dati.getCuaa(), dati.getNome(), dati.getCognome())
				: String.format(PREFISSO_OGGETTO_DICHIARAZIONE_CONSUMI + "%s - %s - %s", dati.getAnno(), dati.getCuaa(),
		...
		return new DocumentDto().setDocumentoPrincipale(documentoByteAsResource)
				.setAllegati(!CollectionUtils.isEmpty(allegati) ? allegati : null).setMetadati(metadatiDto);
	}
	
	protected String getFilename(Long idRichiesta) {
		return String.valueOf(idRichiesta).concat(SUFFISSO_NOME_FILE_DICHIARAZIONE_CONSUMI).concat(".pdf");
	}
	...
	public void aggiornaDomanda(ProtocollaDocumentoUmaDto dati, String numeroProtocollo) {
		try {
			logger.info("Aggiornamento Numero di protocollo {} per dichiarazione consumi {}", numeroProtocollo,
					dati.getId());
			DichiarazioneConsumiModel dichiarazione = dichiarazioneConsumiDao.findById(dati.getId())
					.orElseThrow(() -> new EntityNotFoundException(
							String.format(" Nessuna dichiarazione da aggiornare %s", dati.getId())));
			dichiarazione.setDataProtocollazione(clock.now());
			dichiarazione.setNomeFile(this.salvaDocProtocollato(dati.getId(), dati.getAnno(), dati.getDocumento()));
	...
	private List<ByteArrayResource> estraiAllegatiConsuntivi(DichiarazioneConsumiModel dichiarazioneConsumi) {
		...
		dichiarazioneConsumi.getConsuntivi().stream().filter(consuntivo -> !consuntivo.getAllegati().isEmpty())
				.collect(Collectors.groupingBy(ConsuntivoConsumiModel::getTipoConsuntivo)).entrySet().stream()
				.forEach(entry -> {
					// Per ogni consuntivo prendo gli allegati e li aggiungo alla lista generale con il progressivo per tipo consuntivo
					if (entry.getKey().equals(TipoConsuntivo.AMMISSIBILE) && entry.getValue().stream().anyMatch(
							value -> value.getTipoCarburante().equals(TipoCarburanteConsuntivo.GASOLIO_TERZI))) {
						Integer i = 1;
						for (AllegatoConsuntivoModel allegato : entry.getValue().stream()
								.filter(value -> value.getTipoCarburante()
										.equals(TipoCarburanteConsuntivo.GASOLIO_TERZI))
								.findFirst().get().getAllegati()) {
							var joiner = new StringJoiner("_").add(i.toString()).add("gasolioct")
					...
					if (entry.getKey().equals(TipoConsuntivo.RECUPERO)) {
						...
						for (ConsuntivoConsumiModel consuntivo : entry.getValue()) {
							for (AllegatoConsuntivoModel allegato : consuntivo.getAllegati()) {
								var joiner = new StringJoiner("_").add(i.toString()).add("recupero")
										.add(allegato.getConsuntivoModel().getTipoCarburante().name().toLowerCase())
										.add(dichiarazioneConsumi.getRichiestaCarburante().getCuaa())
										.add(dichiarazioneConsumi.getRichiestaCarburante().getCampagna().toString());
								// Descrizione uguale al nome del file
								allegati.add(
										getAllegatoAsByteArrayResource(allegato.getDocumento(), new StringJoiner("$$")
		...
		if (!CollectionUtils.isEmpty(dichiarazioneConsumi.getClienti())) {
			...
			for (ClienteModel cliente : clienti) {
				for (FatturaClienteModel allegato : cliente.getAllegati()) {
					var joiner = new StringJoiner("_").add(i.toString()).add("contoterzi")
							.add(dichiarazioneConsumi.getRichiestaCarburante().getCuaa())
							.add(dichiarazioneConsumi.getRichiestaCarburante().getCampagna().toString())
