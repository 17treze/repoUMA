...
public class RichiestaCarburanteValidator {
	...
	void validaPresentazioneRichiesta(String cuaa) {
		...
		// controlli uma
		haAlmenoUnaMacchina
		.andThen(nonHaUnaRichiestaInCompilazione)
		.andThen(nonEsisteUnaDichiarazioneConsumiAutorizzata)
		.andThen(nonEsisteUnaDichiarazioneConsumiInCompilazione)
		.accept(cuaa);

		// controlli fascicolo
		FascicoloAgsDto fascicolo = anagraficaClient.getFascicolo(cuaa);
		Optional<Long> idRettificata = ricercaRichiestaCarburanteService.getIdRettificata(cuaa, Long.valueOf(now.getYear()), now);
		...
		if (fascicolo.getDataMorteTitolare() != null && PersonaSelector.isPersonaFisica(cuaa)) {
			...
			Assert.isTrue(!CollectionUtils.isEmpty(anagraficaClient.getTitolariRappresentantiLegali(cuaa)), UMA_01_01_BR1_ERR_MSG);
		}
	}
	...
	private Consumer<String> nonHaUnaRichiestaInCompilazione = cuaa -> {
		RichiestaCarburanteFilter filter = new RichiestaCarburantePagedFilter()
		...
		try {
			...
			throw new IllegalArgumentException("Errore nella ricerca delle richieste di carburante per il cuaa - " + filter.getCuaa());
		}
	...
	private Consumer<String> nonEsisteUnaDichiarazioneConsumiAutorizzata = cuaa -> {
		...
		try {
			...
			throw new IllegalArgumentException("Errore nella ricerca delle dichiarazioni consumi per il cuaa - " + filter.getCuaa());
		}
	...
	private Consumer<String> nonEsisteUnaDichiarazioneConsumiInCompilazione = cuaa -> {
		var filterConsumi = new DichiarazioneConsumiPagedFilter()
				.setNumeroElementiPagina(1)
				.setCampagna(Arrays.asList(Long.valueOf(clock.now().getYear()) - 1))
				.setCuaa(cuaa)
				.setStati(Set.of(StatoDichiarazioneConsumi.PROTOCOLLATA));

		RichiestaCarburanteFilter filterRichieste = new RichiestaCarburantePagedFilter()
				.setNumeroElementiPagina(1)
				.setCuaa(cuaa)
				.setCampagna(Arrays.asList(Long.valueOf(clock.now().getYear()) - 1))
		...
		try {
			...
			throw new IllegalArgumentException("Errore nella ricerca delle richieste di carburante per il cuaa - " + filterRichieste.getCuaa());
		}
		...
		try {
			...
			throw new IllegalArgumentException("Errore nella ricerca delle dichiarazioni consumi per il cuaa - " + filterConsumi.getCuaa());
		}

		if ((richieste != null && !CollectionUtils.isEmpty(richieste.getRisultati())) && (dichiarazioni == null || CollectionUtils.isEmpty(dichiarazioni.getRisultati()))) {
	...
	private Consumer<RichiestaCarburanteModel> validaFascicolo = richiesta -> {
		...
		if (fascicoloValido == null || fascicoloValido.getDataValidazione().compareTo(richiesta.getDataPresentazione()) > 0) {
	...
	private Consumer<RichiestaCarburanteModel> validaFabbisogno = richiesta -> {
		if (richiesta.getBenzina() == null && 
				richiesta.getGasolio() == null && 
				richiesta.getGasolioSerre() == null &&
				richiesta.getGasolioTerzi() == null) {
			throw new IllegalArgumentException(FABBISOGNO_NON_SALVATO);
		}
		if(ricercaRichiestaCarburanteService.getIdRettificata(richiesta.getCuaa(), richiesta.getCampagna(), richiesta.getDataPresentazione()).isPresent()) {
