...
public class PrelieviValidator {
	...
	void validaPrelievo(PrelievoDto prelievo, RichiestaCarburanteModel richiestaCarburante) {
		...
		dataPrelievoAnnoCampagna.andThen(carburanteMaggioreDiZero)
				.andThen(carburantePrelevatoMinoreCarburanteDisponibile).accept(prelievo);
	}
	
	void validaModificaPrelievo(PrelievoDto prelievo, PrelievoModel prelievoOriginale,
			RichiestaCarburanteModel richiestaCarburante) {
		// Calcolo il carburante prelevabile a meno di quello attualmente in modifica
		this.prelevabile = carburanteDtoBuilder.newDto().add(carburanteHelper.calcolaPrelevabile(richiestaCarburante))
				.add(new CarburanteDto()
						...
						.setGasolioSerre(
								prelievoOriginale.getGasolioSerre() != null ? prelievoOriginale.getGasolioSerre() : 0))
				.build();
		
		carburanteMaggioreDiZero.andThen(carburantePrelevatoMinoreCarburanteDisponibile).accept(prelievo);
	}
	...
	private Consumer<PrelievoDto> dataPrelievoAnnoCampagna = dto -> {
		if (dto.getData().getYear() != clock.now().getYear()) {
			throw new IllegalArgumentException(
					"La data indicata per il prelievo non rientra nell'anno di campagna attuale");
		}
	...
	void checkRichiestaCarburanteAutorizzata(RichiestaCarburanteModel richiestaCarburante) {
		if (!richiestaCarburante.getStato().equals(StatoRichiestaCarburante.AUTORIZZATA)) {
			throw new IllegalArgumentException("La richiesta di carburante indicata per il prelievo è nello stato "
					.concat(richiestaCarburante.getStato().name()));
		}
	}
	...
	void checkDichiarazioneConsumiProtocollata(Long idRichiesta) {
		RichiestaCarburanteModel richiesta = richiestaCarburanteDao.findById(idRichiesta)
				.orElseThrow(() -> new EntityNotFoundException(
						"Richiesta con id: ".concat(String.valueOf(idRichiesta)).concat("non trovata")));
		Optional<DichiarazioneConsumiModel> dichiarazioneConsumi = dichiarazioneConsumiDao
				.findByRichiestaCarburante_cuaaAndRichiestaCarburante_campagna(richiesta.getCuaa(),
						richiesta.getCampagna());
		
		if (dichiarazioneConsumi.isPresent()
				&& StatoDichiarazioneConsumi.PROTOCOLLATA.equals(dichiarazioneConsumi.get().getStato())) {
