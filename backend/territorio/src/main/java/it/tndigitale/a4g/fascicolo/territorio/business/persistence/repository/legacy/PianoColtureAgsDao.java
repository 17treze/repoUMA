...
public class PianoColtureAgsDao extends NamedParameterJdbcDaoSupport {
	...
	private String getCodifica(String codice) {
		return String.format("(SELECT %s FROM TABLE(CODIFICA_3_5(pc.ID_COLTURA_1, NULL, :anno))) AS %s ", codice,
				codice);
	}
	...
	private String getQueryPianoColturaleSql() {
		return "SELECT DISTINCT pc.ID_COLTURA_1 AS ID_COLTURA, pc.ID_PARTICELLA, pc.FOGLIO, pc.PARTICELLA, pc.SUB, pc.SUP_ACCERTATA, pc.SUP_DICHIARATA, pc.COD_NAZIONALE, c.TIPO_TITOLO, d.fkid_tipo_doc, t.descrizione, cod.DE_DECODIFICA AS CRIT_MANT, "
				+ getCodifica("CODI_PROD") + ", " + getCodifica("CODI_DEST_USO") + ", " + getCodifica("CODI_USO") + ", "
				+ getCodifica("CODI_QUAL") + ", " + getCodifica("CODI_VARI") + " FROM FASCICOLO.TPIANO_COLTURE pc "
				...
				+ "AND :data BETWEEN c.DATA_INIZIO AND c.DATA_FINE " + "AND :data BETWEEN pc.DT_INIZIO AND pc.DT_FINE "
				+ "AND :data BETWEEN pc.DT_INSERT AND pc.DT_DELETE " + "AND sogg.CUAA = :cuaa " + "AND c.CUAA = :cuaa "
				+ "AND pc.ANNO = :anno " + "AND :data BETWEEN catasez.INIZIO AND catasez.FINE "
