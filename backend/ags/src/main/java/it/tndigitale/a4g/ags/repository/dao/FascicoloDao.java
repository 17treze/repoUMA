...
public class FascicoloDao extends NamedParameterJdbcDaoSupport {
	...
	public List<Fascicolo> getFascicoli(FascicoloFilter fascicolo) {
		...
		logger.debug("chiamata getFascicoli {}, {}, {}", fascicolo.getCuaa(), fascicolo.getDenominazione(), fascicolo.getCaacodici());
		...
		try {
			...
			List<Fascicolo> fascicoloDTOList = getNamedParameterJdbcTemplate().<Fascicolo>query(sql, params, new BeanPropertyRowMapper<Fascicolo>(Fascicolo.class));
	...
	public Fascicolo getFascicolo(Long idFascicolo) throws Exception {
		...
		if (idFascicolo != null) {
			filter = filter.concat(" and f.id_fascicolo = :idFascicolo");
	...
	public boolean checkFascicoloValido(String cuaa) {
		...
		String sql = this.sqlRicercaEsistenzaFascicolo.concat(filter).concat(" AND SCO_STATO IN ('AGGIOR', 'ANOMAL','VALIDO')");
