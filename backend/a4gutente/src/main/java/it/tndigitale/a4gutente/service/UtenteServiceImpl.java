...
public class UtenteServiceImpl implements IUtenteService {
	...
	public UtenteServiceImpl setUtenteCompletoDao(IUtenteCompletoDao utenteCompletoDao, RuoloDao ruoloDao,
			UtenteCompletoCustomDao utenteCompletoCustomDao, SecurityContextWrapper securityContextWrapper,
			IPersonaDao personaDao, IstruttoriaLoader istruttoriaLoader, IDistributoreDao distributoreDao) {
	...
	public List<Profilo> caricaProfiliUtente(String utenza) {
		...
		log.info("Utente: {}", utente.getIdentificativo());
		if (utente == null)
			return null;
		for (A4gtProfilo profilo : utenteCompletoDao.findByIdentificativo(utenza).getProfili()) {
			...
			log.info("Profilo: {}", profiloOut.getDescrizione());
	...
	public void addEnti(A4gtUtente utente, List<A4gtEnte> enti) {
		Set<A4gtEnte> entiDaAssociare = Optional.ofNullable(enti).orElse(new ArrayList<>()).stream()
	...
	public void addProfili(A4gtUtente utente, List<A4gtProfilo> profili) {
		Set<A4gtProfilo> profiliDaAssociare = Optional.ofNullable(profili).orElse(new ArrayList<>()).stream()
				.collect(Collectors.toSet());
	...
	public void removeAziende(A4gtUtente utente) {
		if (utente.getA4grUtenteAziendas() != null && !utente.getA4grUtenteAziendas().isEmpty()) {
			utente.getA4grUtenteAziendas().clear();
			utenteCompletoDao.save(utente);
		}
	}
		...
		if (!distributori.isEmpty()) {
			...
			for (Distributore dist : distributori) {
				A4gtDistributore distributoreAdd = entitaDistributori.stream()
						.filter(d -> d.getId().equals(dist.getId())).findFirst().get();
	...
	public Utente carica(String identificativo) throws Exception {
		A4gtUtente utente = Optional.ofNullable(utenteCompletoDao.findByIdentificativo(identificativo)).orElseThrow(
	...
	public Utente caricaUtenteConInfoPersonaEprofiliDisabilitati(A4gtUtente utente) {
		...
		IstruttoriaEntita istruttoriaEntita = istruttoriaLoader
				.loadLastIstruttoriaByIdentificativoUtente(utente.getIdentificativo());
		return UtenteBuilder.detail(utente, persona)
				.setMotivazioneDisattivazione(
						(istruttoriaEntita != null) ? istruttoriaEntita.getMotivazioneDisattivazione() : null)
				.disableProfiles(getDisabledProfilesFrom(istruttoriaEntita));
	}
	
	@Override
	@Transactional
	public void protocollaPrivacy(String richiedenteNome, String richiedenteCognome, String richiedenteCodiceFiscale,
	...
	private InfoPrivacyDto saveInfoPrivacyInfo(String codiceFiscale, String infoIn, MultipartFile documento,
	...
	public CsvFile getUtentiCsv() throws Exception {
		...
		List<UtenteA4gCsv> utentiA4gListCsv = utentiA4gList.stream().map(u -> {
			Optional<UtenteA4gSospesoDTO> data = Optional.ofNullable(utentiA4gSospesiList)
					.orElseGet(Collections::emptyList).stream().filter(s -> s.getUserName().equals(u.getUserName()))
					.findFirst();
			
			UtenteA4gCsv utenteCsv = data.isPresent()
					? UtenteA4gCsv.buildFromDto(u, true, data.get().getMotivoSospensione())
					: UtenteA4gCsv.buildFromDto(u, false, null);
			
			return utenteCsv;
		}).distinct().sorted(Comparator.comparing(UtenteA4gCsv::getUsername)).collect(Collectors.toList());
		
		ObjectWriter objWriter = new CsvMapper().writerFor(UtenteA4gCsv.class).with(UtenteA4gCsv.getSchema());
	...
	public List<Distributore> caricaDistributoriUtente() {
		...
		if (utente != null) {
			...
			Optional<A4gtProfilo> profiloIstruttoreUma = utente.getProfili().stream()
			...
			if (profiloIstruttoreUma.isPresent()) {
				Set<A4gtDistributore> distributori = distributoreDao.findAll().stream()
						.filter(d -> clock.now().isAfter(d.getDataInizio())).filter(d -> {
	...
	public Distributore caricaDistributoreUtente(Long id) {
		...
		if (utente != null) {
			...
			if (profiloIstruttoreUma.isPresent()) {
				...
				Optional<Distributore> distributoreDtoOpt = DistributoreBuilder
						.from(Collections.singleton((distributoreOpt.get()))).stream().findFirst();
			...
			Optional<Distributore> distributoreOpt = DistributoreBuilder.from(utente.getA4gtDistributori()).stream()
					.filter(d -> d.getId().equals(id)).collect(CustomCollectors.collectOne());
