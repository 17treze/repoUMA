...
public class UtenteController {
	...
	public UtenteController setComponents(IUtenteService utenteService,
										  IDomandaRegistrazioneService domandaService) {
	...
	public Utente carica(@ApiParam(value="Identificativo dell'utente", required = true)
						 @PathVariable(value = "identificativo") String identificativo) throws Exception {
	...
	public Utente caricaMieiDati(
		...
		if (headerProperty != null && !headerProperty.trim().isEmpty()) {
			...
			A4gtUtente utenteBaseDati = a4gtUtenteDao.findByIdentificativo(principalName);
	...
	public Distributore getDistributoreById(@PathVariable(required = true) Long id) {
	...
	public Boolean avviaProtocollazioneUtente (
			@ApiParam(value = "nome utente") 
			@RequestPart(value = "nome") String nome,
			@ApiParam(value = "cognome utente") 
			@RequestPart(value = "cognome") String cognome,
			@ApiParam(value = "codice fiscale utente") 
			@RequestPart(value = "codiceFiscale") String codiceFiscale,
			...
			@ApiParam(value = "Documento da protocollare")
			@RequestPart(value = "documento") MultipartFile documento, 
			@ApiParam(value = "Allegati al documento da protocollare")
	...
	public List<UtenteProfiloA4gDto> getAllUtentiA4gBackofficeViticolo(@RequestParam(required = false) List<ProfiloUtente> profiliToSearch) throws Exception {
