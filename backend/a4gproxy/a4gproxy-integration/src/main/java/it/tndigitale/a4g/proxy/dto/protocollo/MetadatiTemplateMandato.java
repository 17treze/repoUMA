...
public enum MetadatiTemplateMandato {
	...
	NR_REPERTORIO("Nr. repertorio"), CAA("CAA di riferimento"), CUAA("CUAA"), TIPO_DOCUMENTO("Tipo documento");
	...
	public static MetadatiTemplateMandato fromString(String text) {
		return EnumSet.allOf(MetadatiTemplateMandato.class).stream().filter(e -> e.getOriginalFieldName().equals(text)).findAny()
